...
//			});
			HWH5.close();
		}else if(isInWeChatWorkMAC()){
			//MAC 电脑在企业微信内不能自动关闭
			wechatwork_closeWindows();
		}
...
//MAC 电脑
function isInWeChatWorkMAC(){
	var ua = window.navigator.userAgent;
	ua = ua.toLowerCase();
	if(ua.indexOf('mac ')>-1 && ua.indexOf('wxwork')>-1 ){
		return true;
	}
	return false;
}
function wechatwork_closeWindows(){
	var localUrl = encodeURIComponent(window.location.href.split('#')[0]);
	var url = Com_Parameter.ContextPath+"third/wxwork/jsapi/wxJsapi.do?method=pcJsapiSignature&pUrl="+localUrl+"&url="+localUrl;
	$.ajax({
		type: "POST",
		url: url,
		async:false,
...
function Com_OpenWindow(url, target, winStyle, keepUrl){
	...
	// 门户组件链接点击后置灰处理 #185789
	if(window.on_openWindowEvent){
		on_openWindowEvent(url);
	}
...
/***********************************************
 功能：会话校验（仅仅是做会话校验，不作其它处理），__Submit里面有会话校验逻辑，但是在页面表单信息校验时，可能会调用到后台接口，导出该功能失效，现将会话校验功能提前到最优先处理
 ***********************************************/
function Com_SessionValidate() {
	var top = Com_Parameter.top || window.top;
	var canSubmit = true;
	// 会话校验（仅仅是做会话校验，不作其它处理），__Submit里面有会话校验逻辑，但是在页面表单信息校验时，可能会调用到后台接口，导出该功能失效，现将会话校验功能提前到最优先处理
	if (typeof (seajs) != 'undefined' && typeof (top['seajs']) != 'undefined') {
		try {
			$.ajax({
				url: Com_Parameter.ContextPath + 'sys/common/sessionValidate.jsp',
				type: 'POST',
				dataType: 'text',
				async: false,	// 同步
				error: function () {
					// 这里不能使用EKP的弹窗组件，因为连接不上服务器时，无法加载弹窗组件
					top.alert(Com_Parameter.ServiceNotAvailTip);
					canSubmit = false;
					return false;
				},
...
function lockerErrorTip(data,method){
	seajs.use(['lui/dialog', 'lang!'], function (dialog, lang) {
        var __openMutilEdit = false,__innerHashChange = false;
		// 如果模块开启多人在线编辑功能，功能替换
		var _currModel = $("[name='componentLockerVersionForm.fdModelName']").val();
		// 可能版本锁未升级，则引用模块就不会存在 #189097
		if(_currModel && typeof _referenceModel !== "undefined" && _referenceModel && _referenceModel.indexOf(_currModel) > -1){
			__openMutilEdit = true;
			if(typeof _hasAnyChange !== "undefined" && _hasAnyChange){
				__innerHashChange = true;
			}
		}
		...
		var versionValue = fdVersionObj.val();

		// 无审批任务提醒
		if (errorTips == "no_lbpm_task") {
			dialog.alert({"title": lang["sys.common.already.update"],
				"html": lang["sys.common.lock.tip5"],
				"buttons": [{
					name : lang['button.close.refresh'],
					value : true,
					focus : true,
		...
		// 开启了多人编辑版本锁，且无变化，则直接提交
		if(__openMutilEdit && !__innerHashChange){
			if(versionValue){
				// 版本跨度（eg.同一时刻，第三个人提交，距离第一个人提交，则跨度 = 2）,直接增加到最新版本一次提交，而非多次点击递增提交 #188479
				var pastVersion = data_arr.length >=2 && data_arr[2];
				var valueInt = parseInt(versionValue);
				if (isNaN(valueInt)) {
					valueInt = 1;
				}
				var spanV = parseInt(pastVersion);
				if (!pastVersion || isNaN(spanV)) {
					spanV = 1;
				}
				valueInt = valueInt + spanV;
			...
			// 如果模块开启多人在线编辑功能，按钮功能替换
			if(__openMutilEdit && __innerHashChange){
				errorTips = "<p style='font-size: 16px;font-weight: bold;'>"+lang["sys.common.lock.tip1"]+"</p></br>" +
					'<p>'+lang["sys.common.lock.tip2"]+'</p></br>'+
					'<p>'+lang["sys.common.lock.tip3"]+'</p>';
				buttons[1] = {
					name : lang["sys.common.lock.tip4"],
					value : true,
					focus : true,
					fn : function(value, dialog) {
						Com_Parameter.isSubmit = false;
						if(window.showDiffDetail4Submit){
							showDiffDetail4Submit();
						}
						dialog.hide(value);
					}
				};
...
function Com_SetOuterHTML(obj, htmlCode){
	if(Com_Parameter.IE){
		...
		if(htmlCode==""){
			...
			// #194356 节点展开渲染，展示真实名称
			if(df.textContent){
				var _elements = df.childNodes;
				for (var z = 0;z < _elements.length; z++) {
					var spanText = _elements[z].getElementsByClassName('_textName_');
					for (var i = 0; i < spanText.length; i++) {
						var _text = spanText[i].textContent;
						// 重新设置html真实内容
						spanText[i].textContent = Com_encodeHTML(_text);
					}
				}
			}
...
/**
 * 以下为demo定制：访问量统计相关 3秒之类的重复加载不统计
 */
var buryingPointUrl = "https://soe.landray.com.cn:8089/statistics/kafka/buryingPoint";
var baiduUrl = "https://hm.baidu.com/hm.js?4d829b71a81ae4cea109b50ec0d9b4f3";
var pointSource= "ekpdemo";
window.onload = function () {
    const script = document.createElement('script');
    script.src = '/resource/js/common_demo.js';
