;(function (window, factory) {
    if(document.body){
        ...
        style.type = "text/css";
        try{
            style.appendChild(document.createTextNode(".water-mark-wrapper{position:fixed;left:0;top:0;width:100%;height:100%;overflow:hidden;pointer-events:none;z-index:999999999;}.water-mark{position:absolute;overflow:hidden;z-index:99999999999999;pointer-events:none;cursor:default;text-align:center;display:block;-webkit-user-select:none; -moz-user-select:none;-ms-user-select:none;user-select:none;word-break: break-all}"));
        }catch(e){
            style.styleSheet.cssText = ".water-mark-wrapper{position:fixed;left:0;top:0;width:100%;height:100%;overflow:hidden;pointer-events:none;z-index:999999999;}.water-mark{position:absolute;overflow:hidden;z-index:9999999;pointer-events:none;cursor:default;text-align:center;display:block;-webkit-user-select:none; -moz-user-select:none;-ms-user-select:none;user-select:none;word-break: break-all}";
        }
        var head = document.getElementsByTagName("head")[0];
        head.appendChild(style);
    }else
        document.write('<style>.water-mark-wrapper{position:fixed;left:0;top:0;width:100%;height:100%;overflow:hidden;pointer-events:none;z-index:999999999;}.water-mark{position:absolute;overflow:hidden;z-index:9999999;pointer-events:none;cursor:default;text-align:center;display:block;-webkit-user-select:none; -moz-user-select:none;-ms-user-select:none;user-select:none;word-break: break-all}</style>');
    ...
    WaterMark.prototype.render = function () {
        ...
        var opts = this.opts;
        var text = [];
        for (var ii = 0; ii < opts.context.length; ii++) {
            text.push(opts.context[ii].value);
        }
        //获取页面最大宽度
        var pageW;
        var innerHeight;
        ...
        let domTemp;
        if(this.tarDom) {
            domTemp = document.createDocumentFragment();
        }
        else {
            domTemp = document.createElement('div');
            domTemp.setAttribute('class','water-mark-wrapper');
        }
        var x;
        var y;
        // 登录页计算最后一行会超出屏幕导致底部空白 #183610
        ...
        } else if (document.getElementsByClassName("water-mark-wrapper").length == 0) {
    ...
    function removeElement(_element){
        if ( _element.remove ) {
            if(_element.parentNode && _element.parentNode.className === 'water-mark-wrapper'){
                _element.parentNode.remove();
            } else{
                _element.remove();
            }
        } else {
            ...
            if (_parentElement) {
                if(_parentElement.parentNode && _parentElement.className === 'water-mark-wrapper'){
